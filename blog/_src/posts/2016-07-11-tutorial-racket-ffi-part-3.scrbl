...
As in the second post, let's start with some prologue code that just establishes
...
@nested[@tt{$ raco pkg install ffi-definer-convention}]
...
(define-ffi-definer define-cairo cairo-lib
  (code:comment "describes how to transform from Racket to C ids")
...
from my package that has a @tt{#:make-c-id} keyword that lets you specify
...
objects this time. A path is defined as a struct with the following structure:
...
To manipulate paths, we want to define a C type for this. But before that, it's
useful to have C types for the types of values in the path struct's fields. First,
let's just say that a @tt{cairo_status_t} is an integer type:
...
distinguishing various statuses. Next, the data field of a path struct is an
array of path data objects. Each path data object is a @tt{cairo_path_data_t},
...
Helpfully, the FFI library comes with support for unions too with the
...
(define _cairo_path_data_t
  (_union (code:comment "the header case")
          (_list-struct _cairo_path_data_type_t _int)
          (code:comment "the point case")
          (_list-struct _double _double)))
]

First, there's a new type constructor here so let me explain that.
...
the @racket[_cairo_path_data_t] type. Now the thing about union types is
that the programmer has to distinguish the cases in the union manually
on the Racket-side. Let me illustrate this with some code:

@examples[#:eval ev #:label #f
(code:comment "create a union from a list of doubles")
(define a-union-val
  (cast (list 1.3 5.8) (_list-struct _double _double) _cairo_path_data_t))
a-union-val
]

These two lines first construct a union object (via the @racket[_cairo_path_data_t]
type) using a @racket[cast]. A cast is just an operation that lets you coerce
from one C type to another. We use it in this example since it's just an easy way
to generate a union object.

The second line just shows that a union prints as an opaque object. You can't do
...
@examples[#:eval ev #:label #f
(code:comment "correct (matches constructions)")
...
lucky in that case, but in general you can have silent failures where the
data is nonsense.
...
field or variable that indicates the variant. Alternatively, there may be some
ordering of cases.
...
In this type, we have specified the array as just a @racket[_pointer].
...
(do-cairo (λ (ctx)
            (code:comment "Do stuff to make the current path non-empty")
            (cairo-move-to ctx 0.0 0.0)
            (cairo-line-to ctx 100.0 100.0)
            (code:comment "Get the current path")
            (set! a-path (cairo-copy-path ctx))
            (code:comment "Stroke clears the path so do it last")
            (cairo-stroke ctx)))

a-path
]

Now note that @racket[cairo-copy-path] gives us a pointer to a path struct
rather than a path struct directly. So now we'll get back to manipulating
pointers. The most useful function for pointers is @racket[ptr-ref], which
lets you dereference a pointer and access it at some other C type.
...
@examples[#:eval ev #:label #f
(define array
  (ptr-ref (second simple-path)
           (_array/list _cairo_path_data_t (third simple-path))))
array
]

The elements of the array are all unions, as we would expect. This is
a bit cumbersome to use, however. We have to know the structure of the
array and reference the correct variant appropriately:

@examples[#:eval ev #:label #f
(union-ref (first array) 0)
(union-ref (second array) 1)
(eval:error (union-ref (second array) 2))
]
...
the number of data elements specified in the header element (e.g., 2
in the example above) and convert them appropriately.
...
In the Racket to C case, we just extract the pointer field of the struct. In
...
@racket[match-define] form is a very convenient form that lets you define
potentially multiple variables using a pattern. The first argument clause
...
Note: technically @racket[make-do-sequence] actually takes a thunk which
...
(define (get-points ptr pos num-points)
  (for/list ([i (in-range num-points)])
    (union-ref (ptr-ref ptr _cairo_path_data_t (+ pos i))
               1)))
]
...
Each header specifies the length, so we can just loop in @racket[get-points]
...
to deal with union types and custom C types. If not, see the FFI reference
