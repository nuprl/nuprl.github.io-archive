...
Tags: Racket, FFI, by Asumu Takikawa
...
@racketblock[
(cairo-set-dash (list 50.0 10.0 10.0 10.0) 4 -50.0)
]
...
@racketblock[
(cairo-set-dash (list 50.0 10.0 10.0 10.0) -50.0)
]
...
in an value to use as an output, we specify the @racket[o] mode to @racket[_ptr].
...
(define (fit-text str)
  ...
  (define scale (/ (- 256.0 padding) width))
  (cairo-scale txt-ctx scale scale)
  (cairo-show-text txt-ctx str))]

And to conclude part 2 of this tutorial here's how some text will render
with the new @racket[fit-text] function:
